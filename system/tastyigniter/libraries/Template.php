...
//			$this->CI->output->cache($this->CI->config->item('cache_time'));
	...
	protected function _buildNavMenu($nav_menu = array(), $has_child = 0, $recursive = false) {
		$levels = array('', 'nav-second-level', 'nav-third-level');

        $parent_li = '';
        if (!$recursive) $parent_li = 'parent-li-menu';
        ...
        foreach ($nav_menu as $menu) {
            ...
            if (($key_menu == 0 )&& !$first_item && !$recursive) {
                $first_item = true;
                $first_item_padding = 'm-t-30';
            }

            $key_menu = $key_menu + 1;

			$out .= '<li class="'.$parent_li.' '.$first_item_padding.'">'.$this->_buildNavMenuLink($menu);

			if (isset($menu['child']) AND is_array($menu['child'])) {
				$has_child += 1;

				$child_links = $this->_buildNavMenu($menu['child'], $has_child, true);
				$out .= '<ul class="sub-menu '. $levels[$has_child] .'">' . $child_links . '</ul>';
	...
	protected function _buildNavMenuLink($menu_link = array()) {
		...
		if (isset($menu_link['icon']) AND isset($menu_link['title'])) {
			$out .= '<span class="title">'.$menu_link['title'].'</span>';
		} else {
			$out .= '<span class="title">'.$menu_link['title'].'</span>';
		}
		...
		if (isset($menu_link['icon'])) {
			$out .= '<span class="icon-thumbnail "><i class="fa '.$menu_link['icon'].' fa-fw"></i></span>';
        } else {
            $characters = '';
            $title_arr = explode(' ', $menu_link['title']);
            foreach ($title_arr as $title_val) {
                $characters .= substr($title_val, 0, 1);
            }
            $characters = strtolower($characters);

			$out .= '<span class="icon-thumbnail ">'.$characters.'</span>';
		}
